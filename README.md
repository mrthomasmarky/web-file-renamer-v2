...
3.  **Host the Application**:
    ...
    -   For local testing, use a tool like `http-server` with HTTPS:
...
1.  Click the **Upload Files** button or drag and drop files into the input area.
2.  Files appear in the preview list below, showing original and new names (initially identical).
...
1.  After configuring a renaming mode, the preview list shows the new names.
2.  Click **Rename & Download** to generate a ZIP file (`renamed_files.zip`) with the renamed files, using the currently selected mode (first checked option).
